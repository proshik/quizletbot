    ...
    private fun buildInlineKeyboard(showAllLine: Boolean,
        ...
        if (items.size in 5..8) {
            if (firstElemInGroup < 5) {
                navigationRow.add(InlineKeyboardButton().setText("·1-4·").setCallbackData("$PAGING_BY_BUTTONS;1"))
                navigationRow.add(InlineKeyboardButton().setText("5-${items.size}").setCallbackData("$PAGING_BY_BUTTONS;5"))
            } else {
                navigationRow.add(InlineKeyboardButton().setText("1-4").setCallbackData("$PAGING_BY_BUTTONS;1"))
                navigationRow.add(InlineKeyboardButton().setText("·5-${items.size}·").setCallbackData("$PAGING_BY_BUTTONS;5"))
            }
            ...
            when (firstElemInGroup) {
                1 -> {
                    navigationRow.add(InlineKeyboardButton().setText("·1-4·").setCallbackData("$PAGING_BY_BUTTONS;1"))
                    navigationRow.add(InlineKeyboardButton().setText("Next").setCallbackData("$PAGING_BY_BUTTONS;5"))
                }
                in items.size - 4..items.size -> {
                    navigationRow.add(InlineKeyboardButton().setText("Prev").setCallbackData("$PAGING_BY_BUTTONS;${firstElemInGroup - 4}"))
                    val text = if (firstElemInGroup == items.size) {
                        "$firstElemInGroup"
                    } else {
                        "$firstElemInGroup-${items.size}"
                    }
                    navigationRow.add(InlineKeyboardButton().setText("·$text·").setCallbackData("$PAGING_BY_BUTTONS;$firstElemInGroup"))
                }
                else -> {
                    navigationRow.add(InlineKeyboardButton().setText("Prev").setCallbackData("$PAGING_BY_BUTTONS;${firstElemInGroup - 4}"))
                    navigationRow.add(InlineKeyboardButton().setText("·$firstElemInGroup-${firstElemInGroup + 3}·").setCallbackData("$PAGING_BY_BUTTONS;$firstElemInGroup"))
                    navigationRow.add(InlineKeyboardButton().setText("Next").setCallbackData("$PAGING_BY_BUTTONS;${firstElemInGroup + 4}"))
                }
            }
        }
    ...
    private fun buildInlineKeyboard(countOfItems: Int, selectedItem: Int): InlineKeyboardMarkup {
        ...
        if (countOfItems > 5) {
            if (selectedItem > 3 && selectedItem < (countOfItems - 2)) {
                numberRow.add(InlineKeyboardButton().setText("<<1").setCallbackData("$PAGING_BY_ITEMS;1"))
                numberRow.add(InlineKeyboardButton().setText("${selectedItem - 1}").setCallbackData("$PAGING_BY_ITEMS;${selectedItem - 1}"))
                numberRow.add(InlineKeyboardButton().setText("·$selectedItem·").setCallbackData("$PAGING_BY_ITEMS;$selectedItem"))
                numberRow.add(InlineKeyboardButton().setText("${selectedItem + 1}").setCallbackData("$PAGING_BY_ITEMS;${selectedItem + 1}"))
                numberRow.add(InlineKeyboardButton().setText("$countOfItems>>").setCallbackData("$PAGING_BY_ITEMS;$countOfItems"))
            } else {
                if (selectedItem < 4) {
                    if (selectedItem == 1) {
                        numberRow.add(InlineKeyboardButton().setText("·1·").setCallbackData("$PAGING_BY_ITEMS;1"))
                    ...
                    for (item in 2..4) {
                        if (selectedItem == item) {
                            numberRow.add(InlineKeyboardButton().setText("·$item·").setCallbackData("$PAGING_BY_ITEMS;$item"))
                        } else {
                            numberRow.add(InlineKeyboardButton().setText("$item").setCallbackData("$PAGING_BY_ITEMS;$item"))
                        }
                    }
            ...
            for (item in 1..countOfItems) {
                ...
                numberRow.add(InlineKeyboardButton().setText(title).setCallbackData("$PAGING_BY_ITEMS;$item"))
            }
        }
